...
def copy_move(src, dst, move: bool=False, overwrite: bool=True):
	"""Copy or move a file or folder.
	- src: file or directory
	- dst: destination directory or full path
	- move: if True performs a move, otherwise copy
	...
	try:
		src_p = Path(src)
		dst_p = Path(dst)
		# If destination is an existing directory, use same basename
