...
Tmp_F_Ext = '.mp4'
Folder = 'C:\\Users\\Geo\\Videos'
# Folder		= 'C:\\Users\\Geo\\Desktop\\_a'
                ...
                if all_good:
                    ...
                    if all_good:
                        ...
                        if all_good:
							cnt -= 1
							queue_list += [The_file]
							message = f' File: {The_file} _OK_ {all_good}\n'
							Succesful_File.write( message )
							Succesful_File.flush()
							sys.stdout.flush()
...
# XXX: Print Banner
                ...
                if _sub['codec_name'] in ('hdmv_pgs_subtitle', 'dvd_subtitle', 'subrip'):
...
#						ff_subtl.extend( [ zzz, 'mov_text' ] )
        ...
        FFM_cmnd = ff_subtl + ff_video + ff_audio
        end_t = datetime.datetime.now()
        print(
...
#	zzz   = "[0:v]select=not(mod(n\," + slice + ")), scale=-1:280, tile=4x4:nb_frames=16:padding=4:margin=4"

	slice = str( round( Tot_Frms / 9.1 ) ) # We have 9 tiles plus a bit more
	zzz   = "[0:v]select=not(mod(n\," + slice + ")), scale=-1:240, tile=3x3:nb_frames=9:padding=3:margin=3"

	if Tot_Frms > 9000 :
		Cmd   = [Execute, '-ss', '00:00:20', '-vsync', 'vfr', '-i', Fmpg_in_file, '-frames', '1', '-vf', zzz , '-y', Sh_fil_name ]
	else :
		Cmd   = [Execute, '-vsync', 'vfr', '-i', Fmpg_in_file, '-frames', '1', '-vf', zzz , '-y', Sh_fil_name ]

	if DeBug :
		print(f' Len :{Tot_Frms} Div / 4 = {slice} hours = {Tot_Frms/3600}')
		print( zzz )
#		print( Cmd )
		input('OK?')
	...
	try :
		if DeBug :
			...
			print ("    |>-:", Cmd[7:-2] )	## XXX:  Skip First 4 and Last 6
	...
	if  not os.path.exists( Sh_fil_name ) :
        message += ' No Out File Error '
        print(message)
        raise Exception('$hit ', message)
	elif os.path.getsize( Sh_fil_name ) < Min_fsize :
